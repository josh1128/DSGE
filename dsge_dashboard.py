...
#      - Policy shock is applied INSIDE the smoothing bracket
...
with st.sidebar:
    ...
    if model_choice == "Original (DSGE.xlsx)":
        ...
        policy_shock_bp_abs = st.number_input(
            ...
            help="Tightening applies +bp; easing applies -bp. Added to the policy TARGET inside (1-ρ)."
        )
...
def simulate_original(
    ...
    """Simulate paths. Policy shock enters the TARGET inside (1 - rho)."""
    ...
    for t in range(1, T):
        ...
        # >>> CHANGE: apply policy shock INSIDE the (1 - rho) bracket <<<
...
try:
    if model_choice == "Original (DSGE.xlsx)":
        ...
        st.markdown("**Taylor Rule (partial adjustment, with inflation gap and policy shock INSIDE (1−ρ))**")
        st.latex(
            r"""
            i_t \;=\; \rho\, i_{t-1} \;+\; (1-\rho)\,\Big( \alpha^\* \;+\; \phi_{\pi}^\*\,(\pi_t - \pi^\*) \;+\; \phi_{g}^\*\,g_t \;+\; \varepsilon^{\text{pol}}_t \Big)
            """
        )
        st.latex(
            r"""
        ...
        with st.expander("Model diagnostics (OLS summaries)"):
            ...
            st.write("**Taylor Rule (with inflation gap)**"); st.text(m_tr.summary().as_text())
