...
#      - Toggle to show policy rate in **levels (% annual)** or deviations (pp).
#   3) Forward-looking NK (expectations): your model from the screenshot
#      - Phillips:  π_t = β E_t(π_{t+1}) + κ x_t + e_t
#      - IS:        x_t = E_t(x_{t+1}) - (1/θ) ( i_{t-1} - E_t(π_{t+1}) ) + u_t
#      - Taylor:    i_t = ρ_i i_{t-1} + (1−ρ_i)( ψ_π E_t(π_{t+1}) + ψ_y E_t(x_{t+1}) ) + v_t
#        with adaptive expectations: E_t(z_{t+1}) = λ · z_{t-1}
# -----------------------------------------------------------
...
st.markdown(
    ...
    "- Use the sidebar to **toggle models** and **variables**."
)
...
class SimpleNK3EqBuiltIn:
    ...
    def irf(self, shock: str = "demand", T: int = 24, size_pp: float = 1.0, t0: int = 0, rho_override: Optional[float] = None):
        ...
        for t in range(T):
            ...
            B_const = p.rho_i * i_lag + ((1 - p.rho_i) * p.phi_pi * p.gamma_pi - p.gamma_pi) * pi_lag + ((1 - p.rho_i) * p.phi_pi - 1.0) * u[t] + e_i[t]
...
# =========================
# NEW: Forward-looking NK (your model)
# =========================
@dataclass
class NKForwardParams:
    beta: float = 0.99      # discount factor in Phillips
    kappa: float = 0.10     # slope of NK Phillips curve
    theta: float = 1.50     # IS sensitivity denominator (1/theta multiplies real rate gap)
    psi_pi: float = 1.50    # Taylor weight on E(pi_{t+1})
    psi_y: float = 0.10     # Taylor weight on E(x_{t+1})
    rho_i: float = 0.80     # policy smoothing
    lam_exp: float = 0.80   # expectations weight: E_t[z_{t+1}] = lam * z_{t-1}

class ForwardLookingNK:
    """
    Uses your displayed equations with simple adaptive expectations:
      E_t(pi_{t+1}) = λ · pi_{t-1},  E_t(x_{t+1}) = λ · x_{t-1}
    and a timing that avoids simultaneity (uses i_{t-1} in IS).
    Units: percentage points (pp) deviations.
    """
    def __init__(self, params: Optional[NKForwardParams] = None):
        self.p = params or NKForwardParams()

    def irf(self, shock: str = "demand", T: int = 24, size_pp: float = 1.0, t0: int = 0, rho_shock: float = 0.0):
        p = self.p
        x = np.zeros(T); pi = np.zeros(T); i = np.zeros(T)
        u = np.zeros(T); e = np.zeros(T); v = np.zeros(T)

        if shock == "demand":  # u_t
            u[t0] = size_pp
        elif shock == "cost":  # e_t
            e[t0] = size_pp
        elif shock == "policy":  # v_t
            v[t0] = size_pp
        else:
            raise ValueError("shock must be 'demand','cost','policy'")
        ...
        for t in range(T):
            if t > t0:
                if shock == "demand": u[t] += rho_shock * u[t-1]
                elif shock == "cost": e[t] += rho_shock * e[t-1]
                elif shock == "policy": v[t] += rho_shock * v[t-1]

            x_lag = x[t-1] if t>0 else 0.0
            pi_lag = pi[t-1] if t>0 else 0.0
            i_lag = i[t-1] if t>0 else 0.0

            # Expectations from previous realized values (adaptive)
            E_pi_next = p.lam_exp * pi_lag
            E_x_next  = p.lam_exp * x_lag

            # IS:    x_t = E_t(x_{t+1}) - (1/θ) ( i_{t-1} - E_t(π_{t+1}) ) + u_t
            x[t] = E_x_next - (1.0 / max(p.theta, 1e-8)) * (i_lag - E_pi_next) + u[t]

            # Phillips:  π_t = β E_t(π_{t+1}) + κ x_t + e_t
            pi[t] = p.beta * E_pi_next + p.kappa * x[t] + e[t]

            # Taylor:    i_t = ρ_i i_{t-1} + (1−ρ_i)( ψ_π E_t(π_{t+1}) + ψ_y E_t(x_{t+1}) ) + v_t
            i_star = p.psi_pi * E_pi_next + p.psi_y * E_x_next
            i[t] = p.rho_i * i_lag + (1.0 - p.rho_i) * i_star + v[t]
...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        ...
        ["Original (DSGE.xlsx)", "Simple NK (built-in)", "Forward-looking NK (expectations)"],
        index=0
    )
    ...
    # baseline level for plotting policy in % level when desired
    neutral_rate_pct = st.number_input(
        "Baseline (neutral) nominal policy rate — % annual",
        value=2.00, step=0.25, format="%.2f",
        help="Used only when you choose to display the policy rate in level (%)."
    )
...
if model_choice == "Original (DSGE.xlsx)":
    with st.sidebar:
        ...
        st.divider(); st.header("Variable selection (include/exclude)")
    ...
    with st.sidebar:
        ...
        units_mode_simple = st.radio("Policy rate units", ["Deviation (pp)", "Level (% annual)"], index=0)

else:
    # Forward-looking NK (expectations)
    with st.sidebar:
        st.header("Forward-looking NK parameters")
        beta = st.slider("β — weight on expected inflation (Phillips)", 0.80, 1.00, 0.99, 0.01)
        kappa_f = st.slider("κ — Phillips slope", 0.01, 0.50, 0.10, 0.01)
        theta = st.slider("θ — IS denominator (higher ⇒ smaller real-rate effect)", 0.2, 5.0, 1.50, 0.05)
        psi_pi = st.slider("ψπ — Taylor weight on E(π_{t+1})", 1.0, 3.0, 1.50, 0.05)
        psi_y = st.slider("ψy — Taylor weight on E(x_{t+1})", 0.00, 1.00, 0.10, 0.01)
        rho_i_f = st.slider("ρi — Policy smoothing", 0.0, 0.98, 0.80, 0.02)
        lam_exp = st.slider("λ — Expectations weight (E[z_{t+1}] = λ z_{t-1})", 0.0, 0.98, 0.80, 0.02)

        st.divider(); st.header("Shock")
        shock_type_fk = st.selectbox("Shock type", ["Demand (u_t in IS)", "Cost-push (e_t in Phillips)", "Policy (v_t in Taylor)"], index=0)
        shock_size_pp_fk = st.number_input("Shock size (pp)", value=1.00, step=0.25, format="%.2f")
        shock_quarter_fk = st.slider("Shock timing t", 1, T-1, 1, 1)
        shock_persist_fk = st.slider("Shock persistence ρ_shock", 0.0, 0.98, 0.80, 0.02)
        units_mode_fk = st.radio("Policy rate units", ["Deviation (pp)", "Level (% annual)"], index=0)

# =========================
# ORIGINAL MODEL (DSGE.xlsx) — loader/fit/sim
# =========================
...
def load_and_prepare_original(file_like_or_path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df = (is_df.merge(pc_df, on="Date", how="inner")
    ...
    if df_est.empty: raise ValueError("No rows remain after dropping NA for required columns.")
    return df, df_est
...
def fit_models_original(df_est: pd.DataFrame, pi_star_quarterly: float,
    ...
    if not is_selected: raise ValueError("Select at least one regressor for IS.")
    X_is = sm.add_constant(df_est[is_selected], has_constant="add"); y_is = df_est["DlogGDP"]
    model_is = sm.OLS(y_is, X_is).fit()

    if not pc_selected: raise ValueError("Select at least one regressor for Phillips.")
    X_pc = sm.add_constant(df_est[pc_selected], has_constant="add"); y_pc = df_est["Dlog_CPI"]
    ...
    X_tr = sm.add_constant(df_tr, has_constant="add"); y_tr = df_est["Nominal Rate"]
    model_tr = sm.OLS(y_tr, X_tr).fit()

    b0 = float(model_tr.params.get("const", 0.0))
    rhoh = float(model_tr.params.get("Nominal_Rate_L1", 0.0)); rhoh = min(max(rhoh, 0.99*-1), 0.99)
    def safe_div(num, den): return num/den if abs(den) > 1e-8 else np.nan
    alpha_star = safe_div(b0, (1 - rhoh))
    bpi = float(model_tr.params.get("Inflation_Gap", 0.0)); bg  = float(model_tr.params.get("DlogGDP", 0.0))
    ...
    return {"model_is": model_is, "model_pc": model_pc, "model_tr": model_tr,
...
def simulate_original(T: int, rho_sim: float, df_est: pd.DataFrame, models: Dict[str, sm.regression.linear_model.RegressionResultsWrapper],
...
try:
    if model_choice == "Original (DSGE.xlsx)":
        ...
        g0, p0, i0 = simulate_original(T, rho_sim, df_est, models_o, means_o, i_mean_dec, real_rate_mean_dec, pi_star_quarterly,
                                       policy_mode=policy_mode)
        gS, pS, iS = simulate_original(T, rho_sim, df_est, models_o, means_o, i_mean_dec, real_rate_mean_dec, pi_star_quarterly,
        ...
        alpha_star = models_o["alpha_star"]; phi_pi_star = models_o["phi_pi_star"]; phi_g_star = models_o["phi_g_star"]; rho_hat = models_o["rho_hat"]

        is_terms = []
        pretty_map_is = {
            "DlogGDP_L1": r"\Delta \log GDP_{t-1}", "Real_Rate_L2_data": r"RR_{t-2}",
            "Dlog FD_Lag1": r"\Delta \log FD_{t-1}", "Dlog_REER": r"\Delta \log REER_t",
            "Dlog_Energy": r"\Delta \log Energy_t", "Dlog_NonEnergy": r"\Delta \log NonEnergy_t",
        }
        for k, v in m_is.params.items():
            if k == "const": continue
            is_terms.append((float(v), pretty_map_is.get(k, k)))
        st.markdown("**IS Curve (\\(\\Delta \\log GDP_t\\))**"); st.latex(build_latex_equation(float(m_is.params.get("const", 0.0)), is_terms, r"\Delta \log GDP_t", r"\varepsilon_t"))

        pc_terms = []
        pretty_map_pc = {
            "Dlog_CPI_L1": r"\Delta \log CPI_{t-1}", "DlogGDP_L1": r"\Delta \log GDP_{t-1}",
            "Dlog_Reer_L2": r"\Delta \log REER_{t-2}", "Dlog_Energy_L1": r"\Delta \log Energy_{t-1}",
            "Dlog_Non_Energy_L1": r"\Delta \log NonEnergy_{t-1}",
        }
        for k, v in m_pc.params.items():
            if k == "const": continue
            pc_terms.append((float(v), pretty_map_pc.get(k, k)))
        st.markdown("**Phillips Curve (\\(\\Delta \\log CPI_t\\))**"); st.latex(build_latex_equation(float(m_pc.params.get("const", 0.0)), pc_terms, r"\Delta \log CPI_t", r"u_t"))
        ...
        parts.append(rf"\pi^\* = {models_o['pi_star_quarterly']:.4f}")
        ...
        P = NKParamsSimple(
            ...
            rho_i=rho_i, rho_x=(0.0 if snapback else rho_x),
            rho_r=rho_r, rho_u=rho_u, gamma_pi=(0.0 if snapback else gamma_pi)
        )
        ...
        i_ylabel = "pp"
        if units_mode_simple == "Level (% annual)":
        ...
        axes[2].axvline(t0, **vline_kwargs); axes[2].set_title("Nominal Policy Rate (i_t)")
        ...
        # Forward-looking NK (expectations)
        label_to_code = {"Demand (u_t in IS)": "demand", "Cost-push (e_t in Phillips)": "cost", "Policy (v_t in Taylor)": "policy"}
        code = label_to_code[shock_type_fk]
        t0 = max(0, min(T-1, shock_quarter_fk - 1))

        P = NKForwardParams(beta=beta, kappa=kappa_f, theta=theta, psi_pi=psi_pi, psi_y=psi_y, rho_i=rho_i_f, lam_exp=lam_exp)
        model_fk = ForwardLookingNK(P)

        h, x0, pi0, i0 = model_fk.irf(code, T, 0.0, t0, shock_persist_fk)
        h, xS, piS, iS = model_fk.irf(code, T, shock_size_pp_fk, t0, shock_persist_fk)

        # Plot (choose units for policy)
        i0_plot, iS_plot = i0.copy(), iS.copy()
        ylabel = "pp"
        if units_mode_fk == "Level (% annual)":
            i0_plot = neutral_rate_pct + i0_plot
            iS_plot = neutral_rate_pct + iS_plot
            ylabel = "%"
        ...
        axes[0].plot(h, x0, linewidth=2, label="Baseline"); axes[0].plot(h, xS, linewidth=2, label="Shock")
        axes[0].axvline(t0, **vline_kwargs); axes[0].set_title("Output Gap (x̂_t, pp)"); axes[0].set_ylabel("pp")
        axes[0].grid(True, alpha=0.3); axes[0].legend()
        ...
        axes[2].plot(h, i0_plot, linewidth=2, label="Baseline"); axes[2].plot(h, iS_plot, linewidth=2, label="Shock")
        axes[2].axvline(t0, **vline_kwargs); axes[2].set_title("Policy Rate (i_t)")
        axes[2].set_xlabel("Quarters ahead"); axes[2].set_ylabel(ylabel)
        axes[2].grid(True, alpha=0.3); axes[2].legend()

        plt.tight_layout(); st.pyplot(fig)

        with st.expander("Forward-looking NK equations (this option)"):
            st.latex(r"\pi_t \;=\; \beta\,E_t(\pi_{t+1}) \;+\; \kappa\,\hat{x}_t \;+\; e_t")
            st.latex(r"\hat{x}_t \;=\; E_t(\hat{x}_{t+1}) \;-\; \frac{1}{\theta}\,\big(i_{t-1} - E_t(\pi_{t+1})\big) \;+\; u_t")
            st.latex(r"i_t \;=\; \rho_i\,i_{t-1} \;+\; (1-\rho_i)\,\big(\psi_\pi E_t(\pi_{t+1}) + \psi_y E_t(\hat{x}_{t+1})\big) \;+\; v_t")
            st.markdown(r"_Adaptive expectations used here_: $E_t[z_{t+1}] = \lambda\,z_{t-1}$ (tunable in sidebar).")
